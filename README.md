...
NuttX不按启动次数决定回退：已激活的槽每次启动都进行镜像校验，正常重启不扣
次数；旧固件留下的零次剩余记录也可继续启动。`tries_remaining`和`successful`
仅保留在磁盘格式中作兼容，不参与当前槽选择。镜像校验失败仍会清除该槽优先级
并尝试另一槽；没有可用槽时进入Fastboot。此策略不检测跳入系统后的运行故障。
