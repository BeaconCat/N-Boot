...
"""Create and inspect redundant N-Boot A/B boot-control records."""
...
def initial_slot(metadata, enabled, priority):
    return {
        "priority": priority if enabled else 0,
        "tries_remaining": 0,
...
def initial_domain(slot_a, slot_b, enable_a=True, enable_b=False):
    return {
        "active_slot": 0 if enable_a or not enable_b else 1,
        "slots": [
            initial_slot(slot_a, enable_a, 15),
            initial_slot(slot_b, enable_b, 14),
        ],
    }
...
def encode_record(generation, domains):
    ...
    checksum = zlib.crc32(record[:CRC_OFFSET]) & 0xFFFFFFFF
    struct.pack_into("<I", record, CRC_OFFSET, checksum)
    return bytes(record)
...
def command_init(args):
    domains = {
        "nuttx": initial_domain(image_metadata(args.nuttx_a),
                                ...
                                enable_a=True,
                                enable_b=args.nuttx_b is not None),
        "amp": initial_domain(image_metadata(args.amp_a),
                              image_metadata(args.amp_b),
                              enable_a=args.amp_a is not None,
                              enable_b=args.amp_b is not None),
    }
...
def command_inspect(args):
    ...
    result = {
        "copies": copies,
        "selected_copy": None if selected is None else selected["copy"],
    }
